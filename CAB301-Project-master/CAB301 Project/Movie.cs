...
public class Movie : IMovie
{
    ...
    {
        // number of available copies is greater than or equals to 1 
        if (availablecopies >= 1)
        {
            // if the member is not in the borrowers list,
            if (!Borrowers.Search(member))
            {
                ...
                availablecopies--;
                //number of borrowed times increases by one
                noborrows++;
                return true;
            }
        }
    ...
    {
        //if the member is in the borrowers list
        if (Borrowers.Search(member))
        {
            //the member is removed from the borrowers list
            Borrowers.Delete(member);
            //number of available copies increases by one
            availablecopies++;
            return true;
        }
        return false;
